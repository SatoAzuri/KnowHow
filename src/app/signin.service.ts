...
export class SigninService {
  ...
  addChapter(magazineid, name, subject) {
    this.chapters.push(
      {
    ...
    this.magazines[magazineid].chapters.push(this.chapters.length-1);
    return (this.chapters.length - 1);
  }

  addContent(chapterid, name, content, picture, video) {
    this.chapters[chapterid].content.push(
      {
        "id": this.chapters[chapterid].content.length,
        "name": name,
        "content": content,
        "pic": picture,
        "video": video        
  ...
  addAssignment(chapterid,question,option1, option2, option3, answer, img)
  {
    var opt = [];
    opt.push(option1);
    opt.push(option2);
    ...
    this.chapters[chapterid].assignment.push(
      {
        "id": this.chapters[chapterid].assignment.length,
        "question": question,
        "pic": img,
        "options": opt,
        "answer": answer        
